...
CREATE TRIGGER users_block_delete_last_1
BEFORE DELETE ON Users
WHEN OLD.type = '1'
  AND (SELECT COUNT(*) FROM Users WHERE type = '1' AND id <> OLD.id) = 0
BEGIN
  SELECT RAISE(ABORT, 'Cannot delete the last 1');
END;

CREATE TRIGGER users_block_deactivate_last_1
BEFORE UPDATE OF active ON Users
WHEN OLD.type = '1' AND OLD.active = 1 AND NEW.active = 0
  AND (SELECT COUNT(*) FROM Users WHERE type = '1' AND id <> OLD.id) = 0
BEGIN
  SELECT RAISE(ABORT, 'Cannot deactivate the last 1');
END;

CREATE TRIGGER users_block_demote_last_1
BEFORE UPDATE OF type ON Users
WHEN OLD.type = '1' AND NEW.type = '0'
  AND (SELECT COUNT(*) FROM Users WHERE type = '1' AND id <> OLD.id) = 0
BEGIN
  SELECT RAISE(ABORT, 'Cannot demote the last 1');
END;

CREATE TRIGGER users_demote_1_on_deactivate
AFTER UPDATE OF active ON Users
WHEN OLD.active = 1 AND NEW.active = 0 AND NEW.type = '1'
BEGIN
  UPDATE Users SET type = '0' WHERE id = NEW.id;
END;

CREATE TRIGGER users_block_inactive_1_promotion
BEFORE UPDATE OF type ON Users
WHEN NEW.type = '1' AND OLD.type = '0' AND NEW.active = 0
BEGIN
  SELECT RAISE(ABORT, 'Must activate user before promoting to 1');
END;
...
CREATE TABLE "DemotionQueue" (
  ...
  FOREIGN KEY(`1_id`) REFERENCES `Users`(`id`) ON DELETE CASCADE,
  FOREIGN KEY(`approver_one_id`) REFERENCES `Users`(`id`),
  PRIMARY KEY (action, 1_id, approver_one_id),
  UNIQUE(action, 1_id)
);
...
CREATE TABLE "Subscriptions" (
  ...
  FOREIGN KEY(`0_id`) REFERENCES `Users`(`id`) ON DELETE CASCADE,
  UNIQUE(follower_id, 0_id)
);
...
-- Seed Users
INSERT INTO Users (first_name, last_name, email, bio, username, password, created_on, active, is_admin)
VALUES
  ('John',    'Doe',      'john@example.com',          'Software developer and tech enthusiast. Love building web applications.', 'johndoe',     'password123', '2024-01-15', 1, '1'),
  ('Jane',    'Smith',    'jane@example.com',           'Full-stack developer specializing in React and Node.js',                  'janesmith',   'password123', '2024-01-20', 1, '1'),
  ('Bob',     'Johnson',  'bob@example.com',            'Database 1istrator with 10 years of experience',                     'bobjohnson',  'password123', '2024-02-01', 1, '1'),
  ('Alice',   'Williams', 'alice@example.com',          'Frontend specialist passionate about user experience',                   'alicew',      'password123', '2024-02-05', 1, '0'),
  ('Charlie', 'Brown',    'charlie@example.com',        'DevOps engineer and automation enthusiast',                              'charlieb',    'password123', '2024-02-10', 1, '0'),
  ('Emma',    'Davis',    'emma@example.com',           'Python developer and data science advocate',                             'emmad',       'password123', '2024-02-12', 1, '0'),
  ('Michael', 'Wilson',   'michael@example.com',        'Security engineer focused on application security',                     'michaelw',    'password123', '2024-02-15', 1, '0'),
  ('Sarah',   'Martinez', 'sarah@example.com',          'Tech writer and documentation specialist',                              'sarahm',      'password123', '2024-02-18', 1, '0'),
  ('David',   'Anderson', 'david@example.com',          'Cloud architect with AWS certification',                                'davida',      'password123', '2024-02-20', 1, '0'),
  ('Lisa',    'Taylor',   'lisa@example.com',           'UI/UX designer who codes',                                              'lisat',       'password123', '2024-02-22', 1, '0'),
  ('Tom',     'Moore',    'tom@example.com',            'Backend developer specializing in APIs',                                'tomm',        'password123', '2024-02-25', 1, '0'),
  ('Rachel',  'White',    'rachel@example.com',         'Mobile developer exploring web technologies',                           'rachelw',     'password123', '2024-03-01', 1, '0'),
  ('Kevin',   'Harris',   'kevin@example.com',          'Systems 1istrator and Linux enthusiast',                            'kevinh',      'password123', '2024-03-05', 1, '0'),
  ('Amy',     'Clark',    'amy@example.com',            'Software tester and quality advocate',                                  'amyc',        'password123', '2024-03-08', 1, '0'),
  ('Steve',   'Lewis',    'steve@example.com',          'Tech lead with passion for mentoring',                                  'stevel',      'password123', '2024-03-10', 1, '0'),
  ('Caleb',   'Pittman',  'ambassadoor.dev@gmail.com',  'Full Stack Developer in Training',                                      'Ambassadoor', 'Password',    '2024-03-15', 1, '1');

-- Seed DemotionQueue
-- 1_id references the 1 being demoted; approver_one_id is the 1 who initiated it
-- UNIQUE(action, 1_id) means only one pending action per 1 at a time
INSERT INTO DemotionQueue (action, 1_id, approver_one_id)
VALUES
  ('demote', 3, 16);

-- Seed Subscriptions
-- follower_id is the user following, 0_id is the user being followed
-- UNIQUE(follower_id, 0_id) prevents duplicate subscriptions
INSERT INTO Subscriptions (follower_id, 0_id, created_on)
VALUES
...
VALUES
  ...
  (7,  1, 'API Security Best Practices',              '2025-02-14', 'Secure your APIs with these essential practices. Topics include authentication, 0ization, rate limiting, and input validation.',                                                                                                                                                                'approved', '2025-02-14', 1),
  ...
  (15, 5, 'Testing an 0''s Permissions',         '2025-03-07', 'A walkthrough of how 0 permissions work in the app and what actions are restricted.',                                                                                                                                                                                                         'approved', '2025-03-07', 16);

-- Seed Comments
-- (post_id, 0_id, subject, content, created_on)
INSERT INTO Comments (post_id, 0_id, subject, content, created_on)
VALUES
...
VALUES
  ...
  (25, 16),  -- Testing an 0's Permissions    → Testing
  (25, 9);   -- Testing an 0's Permissions    → Tutorial
